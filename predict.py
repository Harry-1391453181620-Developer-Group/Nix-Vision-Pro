"""Top-level inference entrypoint with backend dispatch."""
...
def main() -> None:
    ...
    try:
        if backend == "numpy":
            from backends.numpy.predict_backend import main as backend_main
        else:
            from backends.torch.predict_backend import main as backend_main
    except Exception as exc:
        if backend == "torch":
            raise SystemExit(f"Failed to load the PyTorch backend: {exc}. Use `py -3.13 predict.py ...` or `--backend numpy`.") from exc
        raise
    backend_main()
